...
export function ThreeDeadEndsSlide() {
    ...
    return (
        <section className="relative min-h-[100dvh] w-full flex items-center justify-center p-4 md:p-8 bg-black overflow-hidden z-10 border-t border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8 w-full">
                    {points?.map((point, idx) => (
                        <RevealWrapper key={idx} delay={0.2 + idx * 0.1}>
                            <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-2xl md:rounded-3xl h-full flex flex-col items-start hover:border-[var(--color-brand)]/50 transition-colors duration-300 group">
